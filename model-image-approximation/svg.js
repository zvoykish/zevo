...
(function () {
    ...
    SVG.prepare = function (element) {
        ...
        SVG.parser = {
            body: body ||
                element.parent, draw: draw.style('opacity:0;position:fixed;left:100%;top:100%;overflow:hidden'), poly: draw.polyline().node, path: path
        }
    }
    ...
    SVG.regex = {
        ...
        /* parse hex value */, hex: /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i

        /* parse rgb value */, rgb: /rgb\((\d+),(\d+),(\d+)\)/

        /* test hex value */, isHex: /^#[a-f0-9]{3,6}$/i

        /* test rgb value */, isRgb: /^rgb\(/

        /* test css declaration */, isCss: /[^:]+:[^;]+;?/

        /* test for blank string */, isBlank: /^(\s+)?$/

        /* test for numeric string */, isNumber: /^-?[\d\.]+$/

        /* test for percent value */, isPercent: /^-?[\d\.]+%$/

        /* test for image url */, isImage: /\.(jpg|jpeg|png|gif)(\?[^=]+.*)?/i
    ...
    SVG.defaults = {
        ...
        , attrs: {
            /* fill and stroke */
            'fill-opacity': 1, 'stroke-opacity': 1, 'stroke-width': 0, 'stroke-linejoin': 'miter', 'stroke-linecap': 'butt', fill: '#000000', stroke: '#000000', opacity: 1
            /* position */, x: 0, y: 0, cx: 0, cy: 0
            /* size */, width: 0, height: 0
            /* radius */, r: 0, rx: 0, ry: 0
            /* gradient */, offset: 0, 'stop-opacity': 1, 'stop-color': '#000000'
            /* text */, 'font-size': 16, 'font-family': 'Helvetica, Arial, sans-serif', 'text-anchor': 'start'
        }
        ...
        , trans: function () {
            return {
                ...
                /* matrix */, matrix: this.matrix, a: 1, b: 0, c: 0, d: 1, e: 0, f: 0
            }
        }
    ...
    SVG.extend(SVG.Color, {
        ...
        , at: function (pos) {
            ...
            return new SVG.Color({
                r: ~~(this.r + (this.destination.r - this.r) * pos), g: ~~(this.g +
                    (this.destination.g - this.g) * pos), b: ~~(this.b + (this.destination.b - this.b) * pos)
            })
        }
    ...
    SVG.extend(SVG.PathArray, {
        ...
        , parse: function (array) {
            ...
            for (i = 0, il = segs.numberOfItems; i < il; ++i) {
                ...
                if (s == 'M' || s == 'L' || s == 'H' || s == 'V' || s == 'C' || s == 'S' || s == 'Q' || s == 'T' ||
                    s == 'A')
                {
                    if ('x' in seg) {
                        x = seg.x
                    ...
                    {
                        segs.replaceItem(SVG.parser.path.createSVGPathSegArcAbs(x, y, seg.r1, seg.r2, seg.angle,
    ...
    SVG.BBox = function (element) {
        ...
        if (element) {
            try {
                ...
                box = {
                    x: element.node.clientLeft, y: element.node.clientTop, width: element.node.clientWidth, height: element.node.clientHeight
                }
            }
    ...
    SVG.Element = SVG.invent({
        ...
        , extend: {
            ...
            , size: function (width, height) {
                ...
                return this.attr({
                    width: new SVG.Number(p.width), height: new SVG.Number(p.height)
                })
            }
            // Clone element
            , clone: function () {
            ...
            , transform: function (o, v) {
                ...
                if (o.rotation != 0)
                {
                    transform.push('rotate(' + o.rotation + ' ' + (o.cx == null ? this.bbox().cx : o.cx) + ' ' +
                ...
                if (o.x != 0 || o.y != 0)
                {
                    transform.push('translate(' + new SVG.Number(o.x / o.scaleX) + ' ' +
        ...
        , extend: {
            ...
            animate: function (d, ease, delay) {
                ...
                fx.to = function (pos) {
                    ...
                    if (akeys == null) {
                        ...
                        if (element.morphArray && (fx._plot || akeys.indexOf('points') > -1)) {
                            ...
                            /* add size */
                            if (fx._size) {
                                p.size(fx._size.width.to, fx._size.height.to)
                            }

                            /* add movement */
                            box = p.bbox()
                            if (fx._x) {
                                p.move(fx._x.to, box.y)
                            }
                            else if (fx._cx) {
                                p.move(fx._cx.to - box.width / 2, box.y)
                            }

                            box = p.bbox()
                            if (fx._y) {
                                p.move(box.x, fx._y.to)
                            }
                            else if (fx._cy) {
                                p.move(box.x, fx._cy.to - box.height / 2)
                            }
                ...
                if (typeof d === 'number') {
                    ...
                    this.timeout = setTimeout(function () {
                        ...
                        fx.situation = {
                            interval: 1000 / 60, start: start, play: true, finish: start + d, duration: d
                        }
            ...
            , size: function (width, height) {
                if (this.target instanceof SVG.Text) {
                    ...
                    this._size = {
                        width: new SVG.Number(box.width).morph(width), height: new SVG.Number(box.height).morph(height)
                    }
                }
            ...
            , viewbox: function (x, y, width, height) {
                if (this.target instanceof SVG.Container) {
                    ...
                    this._viewbox = {
                        x: new SVG.Number(box.x).morph(x), y: new SVG.Number(box.y).morph(y), width: new SVG.Number(box.width).morph(width), height: new SVG.Number(box.height).morph(height)
                    }
                }
    ...
    SVG.extend(SVG.Defs, {
        ...
        pattern: function (width, height, block) {
            return this.put(new SVG.Pattern).update(block).attr({
                x: 0, y: 0, width: width, height: height, patternUnits: 'userSpaceOnUse'
            })
        }
    ...
    SVG.Ellipse = SVG.invent({
        ...
        , extend: {
            ...
            , size: function (width, height) {
                ...
                return this.attr({
                    rx: new SVG.Number(p.width).divide(2), ry: new SVG.Number(p.height).divide(2)
                })
            }
        ...
        , extend: {
            ...
            x: function (x) {
                ...
                return x == null ? b.x : this.attr({
                    x1: this.attr('x1') - b.x + x, x2: this.attr('x2') - b.x + x
                })
            }
            // Move over y-axis
            , y: function (y) {
                ...
                return y == null ? b.y : this.attr({
                    y1: this.attr('y1') - b.y + y, y2: this.attr('y2') - b.y + y
                })
            }
            // Move by center over x-axis
            , cx: function (x) {
            ...
            , height: function (height) {
                ...
                return height == null ? b.height : this.attr(this.attr('y1') < this.attr('y2') ? 'y2' : 'y1',
                    b.y + height)
            }
                ...
                img.onload = function () {
                    ...
                    if (typeof self._loaded === 'function')
                    {
                        self._loaded.call(self, {
                            width: img.width, height: img.height, ratio: img.width / img.height, url: url
                        })
                    }
                }
    ...
    var sugar = {
            stroke: ['color', 'width', 'opacity', 'linecap', 'linejoin', 'miterlimit', 'dasharray', 'dashoffset'], fill: ['color', 'opacity', 'rule'], prefix: function (t,
                                                                                                                                                                         a)
            {
            ...
            for (var k in o)
            {
                k == 'leading' ?
                    ...
                    k == 'anchor' ?
                        ...
                        k == 'size' || k == 'family' || k == 'weight' || k == 'stretch' || k == 'variant' ||
                            k == 'style' ?
